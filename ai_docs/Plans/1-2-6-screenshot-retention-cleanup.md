...
**Approach:** Create an `ArchiveWorker` class that runs on startup and monthly to compress completed month folders into zip archives. Archive runs after one clear month has passed (e.g., on Dec 1, archive October). On failure, display tkinter dialog with retry options.
