#!/usr/bin/env python3
"""
Story Tree Explorer v1.2 - PySide6 implementation with native per-cell coloring
...
try:
    from PySide6.QtWidgets import (
        ...
        QTextEdit, QFrame, QSplitter, QScrollArea, QDialog, QFileDialog,
        QMessageBox, QMenu, QGroupBox, QStatusBar, QSizePolicy
    )
...
class StatusChangeDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # Header
        header_label = QLabel(f"Changing '{self.node_id}' to status: {self.new_status}")
...
class ClickableLabel(QLabel):
    """A label that emits a signal when double-clicked."""
    doubleClicked = Signal(str)

    def __init__(self, text: str, node_id: str, parent=None):
        super().__init__(text, parent)
        self.node_id = node_id
        self.setStyleSheet("color: #0066CC; text-decoration: none;")
...
class DetailView(QWidget):
    ...
    def __init__(self, app: 'StoryTreeExplorer'):
        super().__init__()
        self.app = app
    ...
    def show_node(self, node_id: str, add_to_history: bool = True):
        ...
        if node.children:
            self._add_children_section(node.children)
        else:
    ...
    def _add_link_field(self, label: str, node_id: str):
        ...
        link_text = f"{node_id}"
        if node:
            title_preview = node.title[:40] + '...' if len(node.title) > 40 else node.title
            link_text += f" - {title_preview}"
    ...
    def _add_children_section(self, children: List[StoryNode]):
        """Add a section with clickable child links."""
        label_widget = QLabel("Children:")
        label_font = label_widget.font()
        label_font.setBold(True)
        label_widget.setFont(label_font)
        ...
        for child in children:
            title_preview = child.title[:50] + '...' if len(child.title) > 50 else child.title
            link_text = f"  {child.id} [{child.status}] - {title_preview}"
...
class StoryTreeExplorer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.node_items: Dict[str, QTreeWidgetItem] = {}  # Maps node_id to QTreeWidgetItem
    ...
    def _setup_ui(self):
        ...
        for status in ALL_STATUSES:
            ...
            filter_group_layout.addWidget(cb)
        ...
        self.view_stack_layout.addWidget(self.tree_view_frame)

        # Detail view frame
        self.detail_view = DetailView(self)
        self.detail_view.closeRequested.connect(self.show_tree_view)
        self.view_stack_layout.addWidget(self.detail_view)
    ...
    def _add_filtered_node(self, node: StoryNode, parent_item: Optional[QTreeWidgetItem],
        ...
        # Create tree item with columns: ID, Status, Title
        ...
        # Apply colors
        status_color = QColor(STATUS_COLORS.get(node.status, '#000000'))
        black = QColor('#000000')
        gray = QColor('#999999')

        if node.id in faded_nodes:
            # Faded ancestor: gray text, but keep status color
            item.setForeground(0, QBrush(gray))  # ID = gray
            item.setForeground(1, QBrush(status_color))  # Status = colored
            item.setForeground(2, QBrush(gray))  # Title = gray

            # Italic font for faded items
            font = item.font(0)
            font.setItalic(True)
            item.setFont(0, font)
            item.setFont(1, font)
            ...
            # Normal node: colored status, black text
            item.setForeground(0, QBrush(black))  # ID = black
            item.setForeground(1, QBrush(status_color))  # Status = colored
            item.setForeground(2, QBrush(black))  # Title = black
