...
**Technical Notes:**
- Use `uiautomation` or `pywinauto` library (same as 8.2)
- Share UI Automation utilities with 8.2 if natural fit, but don't over-engineer
...
- Browsers handled separately in 8.2
...
def _finalize_current_event(self) -> Optional[ActivityEvent]:
    ...
    event = ActivityEvent(
        ...
        url=None,  # URL extraction is handled separately (story 8.2)
        metadata=metadata,  # NEW FIELD
...
**Follow-up work:**
- Story 8.2 (Browser URL Extraction) can reuse UIAutomationWorker pattern
- Story 8.4 (AI Disambiguation) will consume metadata for better categorization
